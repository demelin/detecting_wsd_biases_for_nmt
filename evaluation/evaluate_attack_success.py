...
def evaluate_attack_success(adversarial_samples_path,
    ...
    def _score_and_filter(adversarial_sample_table_entry, attractors_entry, ambiguous_term, ambiguous_form):
        ...
        for true_cluster in adversarial_sample_table_entry.keys():
            for adv_cluster in adversarial_sample_table_entry[true_cluster].keys():
                for sample_entry in adversarial_sample_table_entry[true_cluster][adv_cluster]:
                    # Unpack
                    adv_src = sample_entry[0]
                    true_src = sample_entry[1]
                    true_tgt = sample_entry[2]
                    attractor = sample_entry[3]
                    attractor_scores = sample_entry[4]
                    if samples_from_words:
                        ambiguous_token_ws_loc_adv = sample_entry[5]
                        all_ambiguous_tokens_ws_loc_adv = sample_entry[6]  # list
                        ambiguous_token_loc_seed = sample_entry[7]
                        ambiguous_token_ws_loc_seed = sample_entry[8]  # list
                        tgt_sense_loc_seed = sample_entry[9]
                        tgt_sense_ws_loc_seed = sample_entry[10]  # list
                        ...
                        lm_score_ratio = float(sample_entry[16])
                        provenance_tags = sample_entry[17]
                        adv_translation_line_id = int(sample_entry[18][1])
                        true_translation_line_id = int(sample_entry[19][1])
                    else:
                    ...
                    # Look up alignments and sense mappings
                    adv_alignments = \
                        adv_alignment_table[adv_translation_line_id] if adv_alignment_table is not None else None
                    true_alignments = \
                        true_alignment_table[true_translation_line_id] if true_alignment_table is not None else None
                    ...
                    true_nmt_label, true_translation_sense_clusters, _ = \
                        _get_nmt_label(true_translations[true_translation_line_id],
                                       adv_src,
                                       true_src,
                                       true_tgt,
                                       ambiguous_token,
                    ...
                    adv_nmt_label, adv_translation_sense_clusters, attractor_is_translated = \
                        _get_nmt_label(adv_translations[adv_translation_line_id],
                                       adv_src,
                                       true_src,
                                       true_tgt,
                                       ambiguous_token,
                                       ...
                                       adv_alignments,
                                       is_adv=True,
                                       true_translation=true_translations[true_translation_line_id],
                                       true_alignments=true_alignments)

                    # Assemble table entry
                    new_table_entry = [adv_src,
                                       adv_translations[adv_translation_line_id],
                                       true_src,
                                       true_translations[true_translation_line_id],
                                       true_tgt,
                                       ambiguous_token,
                                       attractor,
                                       attractor_scores,
                    ...
                    if seed_table_to_expand is not None:
                        ...
                        seed_table_to_expand[ambiguous_term][true_cluster][adv_cluster][true_src]\
                            .append([true_translations[true_translation_line_id], true_tgt,
                                     true_translation_sense_clusters, ambiguous_token_loc_seed])

                    # Collect attack success samples
                    if not table_to_expand.get(ambiguous_term, None):
                        table_to_expand[ambiguous_term] = dict()
                    if not table_to_expand[ambiguous_term].get(true_cluster, None):
                        table_to_expand[ambiguous_term][true_cluster] = dict()
                    if not table_to_expand[ambiguous_term][true_cluster].get(adv_cluster, None):
                        table_to_expand[ambiguous_term][true_cluster][adv_cluster] = dict()
                    if not table_to_expand[ambiguous_term][true_cluster][adv_cluster].get(true_src, None):
                        table_to_expand[ambiguous_term][true_cluster][adv_cluster][true_src] = list()
                    table_to_expand[ambiguous_term][true_cluster][adv_cluster][true_src].append(new_table_entry)
                    ...
                    for attractor_deletion_table in [single_attractor_deletion_table, joined_attractor_deletion_table]:
                        ...
                        if not attractor_deletion_table[ambiguous_term].get(true_cluster, None):
                            attractor_deletion_table[ambiguous_term][true_cluster] = dict()
                        if not attractor_deletion_table[ambiguous_term][true_cluster].get(adv_cluster, None):
                            attractor_deletion_table[ambiguous_term][true_cluster][adv_cluster] = dict()
                        if not attractor_deletion_table[ambiguous_term][true_cluster][adv_cluster].get(true_src, None):
                            attractor_deletion_table[ambiguous_term][true_cluster][adv_cluster][true_src] = list()
                        attractor_deletion_table[ambiguous_term][true_cluster][adv_cluster][true_src]\
    ...
    # Check file types
    samples_from_words = '_word_attractors' in adversarial_samples_path
    ...
    with open(true_translations_path, 'r', encoding='utf8') as ttp:
        true_translations = {line_id: line for line_id, line in enumerate(ttp)}
    ...
    if true_alignments_path:
        assert adversarial_alignments_path is not None, 'Missing alignments for adversarial translations!'
    ...
    if adversarial_alignments_path:
        assert true_alignments_path is not None, 'Missing alignments for true translations!'
    ...
    # Iterate over known available adversarial samples
    for term_id, term in enumerate(adversarial_samples_table.keys()):
        ...
        if has_forms:
            for form in adversarial_samples_table[term].keys():
                _score_and_filter(adversarial_samples_table[term][form], attractors_table[term][form], term, form)
        else:
            _score_and_filter(adversarial_samples_table[term], attractors_table[term], term, None)
